...
def check_host_service(process_name):
    ...
    # Use ps aux for CPU and memory usage
        ...
        for line in ps_result.stdout.splitlines():
            if re.search(process_name, line, re.IGNORECASE):
                ...
                if len(parts) >= 11:
                    cpu = float(parts[2])
                    mem = float(parts[3])
                    # You can optionally convert %MEM to MB if you want:
                    # mem_mb = float(parts[5]) / 1024
                # Check STAT for unhealthy states
                stat = parts[7]
...
def get_container_resource_usage():
    """Get CPU and memory usage percent for all running containers."""
    usage = {}
    try:
        result = subprocess.run(
...
def update_metrics():
    ...
    for name, process in HOST_SERVICES.items():
        ...
        if mem is not None:
            service_mem.labels(service=name).set(mem)  # mem is already percentage from ps aux
    ...
    # Expose resource usage for all running containers (percent)
    container_usage = get_container_resource_usage()
    for cname, (cpu, mem_perc) in container_usage.items():
        service_cpu.labels(service=f"container_{cname}").set(cpu)
        service_mem.labels(service=f"container_{cname}").set(mem_perc)
